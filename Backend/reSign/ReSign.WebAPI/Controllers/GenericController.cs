...
namespace ReSign.WebAPI.Controllers
{
        ...
        public abstract partial class GenericController<TEntity, TEditModel, TModel> : ControllerBase, IDisposable
            where TEntity : IdentityEntity, new()
            where TEditModel : class, new()
            where TModel : class, new()
        {
            private bool disposedValue;

            protected GenericController<TEntity> EntityController { get; init; }

            internal GenericController(GenericController<TEntity> controller)
            {
            ...
            public virtual async Task<ActionResult<TModel>> PostAsync([FromBody] TEditModel model)
            {
                ...
                return CreatedAtRoute(new { id = entity.Id }, ToModel(insertEntity));
            }
